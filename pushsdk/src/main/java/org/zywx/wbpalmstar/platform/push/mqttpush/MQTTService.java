...
public class MQTTService implements MqttSimpleCallback {
    ...
    public MQTTService(Context context, String url, String softToken) {
        _context = context;
        brokerHostName = url;
...
//    private short keepAliveSeconds = 5 * 60;
    ...
    public void publishArrived(String topic, byte[] payloadbytes, int qos,
        ...
        if (reData != null && reData.length() > 0) {
            ...
            try {
                ...
                String mAppId = PushReportUtility.getAppID(_context);
                String decryptedData = Rc4Encrypt.decry_RC4(reData, "MOBILEOA");
                PushReportUtility.log("publishArrived decryptedData "+decryptedData);
                json = new JSONObject(decryptedData);
                Log.e("--aaaa--","接受消息"+json);
    ...
    private void handlePushMsg(JSONObject json) throws JSONException {
        String turn = json.optString("turn");
        if ("on".equalsIgnoreCase(turn)) { // 推送4.0版本
            ...
            if (json.has("mdm") && json.getString("mdm") != null) {
                ...
                if ("ok".equals(status)) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        pushData(jsonArray.getJSONObject(i));
                    }
                }
            }
        }
    ...
    private void sendMdmMsg(JSONObject msg) throws JSONException {
        Intent intent = new Intent();
        intent.setAction(ACTION_MDM);
        intent.setPackage(_context.getPackageName());
        intent.putExtra("mdmtoken", msg.getString("mdm"));
        _context.sendBroadcast(intent);
    }

    private void pushDataInfo(JSONObject data) {
        ...
        if (TextUtils.isEmpty(title)) {
            try {
                ...
                PackageInfo pinfo = pm.getPackageInfo(_context.getPackageName(), PackageManager.GET_CONFIGURATIONS);
    ...
    private void pushData(JSONObject text) {
        try {
            ...
            _context.sendBroadcast(intent);//传递过去

            try {
                PushReportAgent.reportPush(_context, value, PushReportConstants.EVENT_TYPE_ARRIVED);// 推送消息到达上报
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean connectToBroker() {
        try {
            // try to connect
            String mAppId = PushReportUtility.getAppID(_context);
            String macAddress = PushReportUtility.getMacAddress(_context).replaceAll(":", "");
